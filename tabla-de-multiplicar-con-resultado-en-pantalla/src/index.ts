...
btnCalc.addEventListener("click", (e) => {
  ...
  for (let i: Number = 1; i <= limit; i++) {
    ...
    li.innerText = `${num} x ${i} = ${result}`; // Se asigna una funcion de escritura de texto y su correspondiente valor.
  ...
  numImput.value = ""; // Se inicializan las variables, sin valor aun.
  numLimit.value = ""; // Se inicializan las variables, sin valor aun.
});
